...
class Place:
  ...
  def __init__(self, side, wall_brick_color, level, over, brick2place_color, hokuyo_x):
        #members
    ...
    self.level = level
    self.hokuyo_x = hokuyo_x
    rospy.init_node('place_algorithm', anonymous=True)
  ...
  def compute_place_pose(self, edge_point): #receives edge point in base_link
    ...
    return self.create_pose(self.hokuyo_x, y, height, "/base_link") #VERIFICAR ESTA POSE 
...
def main(args):
  #side, wall_brick_color, level, over, brick2place_color || 3 first parameters refer to the reference brick
  ic = Place("left", "blue", 1, True, "green")
  try:
    ic.execute()
