...
class Place:
  ...
  def __init__(self):
        #members
    # self.brick2place_color = rospy.get_param('brick_to_place')
    # self.over = rospy.get_param('over')
    # self.side = rospy.get_param('side')
    # self.wall_brick_color = rospy.get_param('reference_brick')
    # self.level = rospy.get_param('level')
    self.brick2place_color = "orange"
    self.over = False
    self.side = "right"
    self.wall_brick_color = "orange"
    self.level = 1

    self.t = tf.TransformListener() 
    self.edge = edge_detector(self.side, self.wall_brick_color, self.level)
